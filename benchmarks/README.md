...
| MemOS（取回内容） | 50.8% | FTS5 + 向量，与我们同路线 |
...
1. **Cognee 的图谱路径是唯一显著领先的**。扣掉作答模型带来的约 5 点，真实领先
   ...
   作答」而非「返回原文」。
2. **中间五家分不出高下**。其中 MemOS 与我们同技术路线（FTS5 + 向量），它
   50.8% 我们 46.7%，差距小于噪音。**这说明「词法+向量检索原文」这条路的天花板
   就在 50% 上下，而我们已经在天花板上** —— 今天把中文召回从 47.7% 提到 58.9%
   （+11 点），端到端一点没动。
