# Memory retrieval benchmark

Answers one question: when the agent goes looking for something it stored, does
it find it? Nothing here calls a model — [LoCoMo][locomo] labels which dialogue
turn holds each answer, so a hit is a fact rather than a judgement, and the
whole thing runs offline in about a minute.

```bash
curl -sSLo benchmarks/locomo10.json \
  https://raw.githubusercontent.com/snap-research/locomo/main/data/locomo10.json

python benchmarks/memory_recall.py    # our search
python benchmarks/bm25_baseline.py    # BM25, for reference
```

Every dialogue turn becomes one memory entry; every question becomes one query.
A question counts as answered if the labelled turn comes back at all.

## Why the baseline is here

A recall figure on its own cannot say whether the number is the task being hard
or the engine being weak, and those call for different responses. BM25 is the
standard keyword baseline and needs no embeddings or services — the same class
of thing this project could ship — so it marks what is reachable without
changing the architecture.

## What it has caught

The first run scored **0.0% at every depth**. The whole query was being matched
as a single quoted phrase, which meant a natural question could only match a
turn containing that question verbatim. Quoting had been added to stop FTS5
reading `core.py` as syntax; it also made the engine useless, and nothing said
so until this was run. Splitting the query into OR-ed terms took recall@10 to
**65.3%**, past BM25's 57.9%.

Worth re-running after any change to query construction, tokenisation, or
ranking.

| | recall@1 | recall@5 | recall@10 |
| --- | --- | --- | --- |
| whole question | 35.3% | 57.0% | 65.3% |
| BM25 baseline | 26.1% | 49.3% | 57.9% |

## Bilingual

```bash
python benchmarks/bilingual_recall.py
```

LoCoMo is English, and English alone is not what this has to work for: a real
store mixes Chinese prose with English identifiers inside single entries and is
queried the same way. That path arrived as a fallback — used only when the index
came back empty — so a mixed query answered its English half and dropped the
Chinese one, which is the commonest shape of query there is.

The fixture is written by hand; there is no public Chinese long-conversation
memory set to sample from. So this is a regression target rather than a
comparison: it says whether a change made retrieval worse, not whether
retrieval is good.

Several queries expect **two** entries — one matched by an English identifier,
one by Chinese prose — and score only if both come back. That detail is the
whole point. The first version scored each query on finding any one relevant
entry, and gave an identical 100% before and after the bilingual fix; it was
worthless until it was rewritten to fail.

[locomo]: https://github.com/snap-research/locomo


## vs_mem0_retrieval.py — 与 mem0 的检索对比

只比检索器，不比整个记忆系统。两边存入完全相同的原文（mem0 用 `infer=False`
关掉它的 LLM 抽取），打分用 LoCoMo 自带的 `evidence` 证据轮标注，不看答案字符串
——否则"存原文"的一方会因为标准答案本就是原文子串而白捡分数。

这也意味着它**测不到 mem0 的抽取层**，而那是 mem0 的核心。任何"谁的记忆系统更好"
的结论都不能从这个脚本得出。

运行（mem0 不是本项目依赖）：

    BENCH_SYS_PATH=/path/to/mem0-install python benchmarks/vs_mem0_retrieval.py

中文对比需要先用翻译脚本生成 `locomo10_zh.json`：证据标注是轮次 id，翻译不会
破坏它，所以除语言外所有变量都被控制住。中文必须配多语言嵌入模型
（`BENCH_EMBED`），用英文专用模型测中文测的是模型选型不是检索能力。

### 两个必须设对的参数

mem0 的 `search()` 参数是 `top_k` 不是 `limit`；传 `limit=` 会被 `**kwargs`
静默吞掉。`threshold` 默认 0.1，中文场景下正确命中的分数常在 0.101 附近，默认值
会把结果砍到只剩一条——那时测的是阈值，不是召回。两处都设错过，都产生过看起来
可信但完全错误的结论。

### 结果（2 段对话，788 轮，302 问，2026-08）

英文：

| 检索器 | recall@1 | recall@3 | recall@8 |
|---|---|---|---|
| OpenKimo (FTS5 词法) | 36.4% | 52.6% | 64.6% |
| mem0 纯向量 (bge-small-en) | 25.5% | 44.4% | 58.3% |
| mem0 混合 (向量+BM25) | 36.4% | 57.3% | **68.5%** |

中文（同批对话译入，证据标注不变）：

| 检索器 | recall@1 | recall@3 | recall@8 |
|---|---|---|---|
| OpenKimo (FTS5 词法) | 19.5% | 34.8% | 47.7% |
| mem0 纯向量 (多语言 MiniLM) | 20.2% | 38.4% | **54.6%** |
| mem0 混合 (向量+BM25) | 18.5% | 31.8% | 49.3% |

结论：**纯词法检索在两种语言上都落后于带稠密向量的方案**（英文 -3.9、中文
-6.9，均为 recall@8，也是 `search` 工具实际返回的条数）。BM25 那一半在英文上
值 +10 个点，在中文上是负收益（49.3 对 54.6）——空格分词在中文上不成立，这一点
与选哪个嵌入模型无关。

我们自己的中文比英文低 17 个点（47.7 对 64.6）：中文检索是这套方案最弱的一环。

### 中文排序修复后（2026-08）

scan 此前按「命中了几个 needle」排序，而中文 needle 是查询的每个双字窗口——
问句的语法（什么、时候、怎么）与内容（互助、小组）等权。英文靠停用词表挡住了
这件事，中文没有表。改为按逆文档频率加权，并让含中文的查询以 scan 为主、FTS5
只补空位（纯拉丁查询走原路径不变）：

| | recall@1 | recall@3 | recall@8 |
|---|---|---|---|
| 中文 修改前 | 19.5% | 34.8% | 47.7% |
| 中文 修改后 | 32.8% | 46.4% | **58.9%** |
| 英文 修改前后 | 36.4% | 52.6% | 64.6% |

中文 +11.2 点（recall@8），英文一点不动，仍然零依赖。对比 mem0：中文 58.9%
对 54.6%（领先），英文 64.6% 对 68.5%（落后）。

调优过程中有一个必须记下来的教训：最初的版本还会剪掉出现在 5% 以上条目里的
needle，在两千条的 benchmark 上多赚半个点，在真实记忆库上是灾难——真实库只有
几十条，5% 不足 1，于是**凡是真正匹配上的 needle 都被剪掉**，只留下匹配不到
任何东西的。benchmark 抓不到这个，因为它从没在真实规模的库上跑过。
`TestRankingOnASmallStore` 就是为这件事留的。


## 端到端问答，以及抽取层（2026-08）

召回率只对存原文的系统公平：一个记忆系统若靠合成作答，它给出的正确答案可能不
对应任何单一原文轮，用「证据轮是否进 top-k」去打分等于用尺子量重量。所以加了
`judge.py`——同一个问题给每个系统，各自返回什么就用什么作答，再由一个不知道答
案出自哪个系统的模型对照标准答案判分。

必须先修的一处：LoCoMo **23% 的答案是日期**，而日期在 `session_N_date_time`
字段里、不在任何轮次文本里。不把会话时间戳带进语料，检索对了也答不出来——补上
之后所有系统的准确率翻倍。

### 结果（2 段对话，120 题，重复 6 次）

| 方案 | 均值 | 极差 |
|---|---|---|
| Cognee（图谱自答） | 64.4% | 2.5 |
| OpenKimo — 换 k3 作答（对照） | 54.4% | 3.3 |
| mem0（qdrant 混合） | 53.5% | 6.6 |
| OpenKimo（检索原文轮次） | 49.2% | 4.1 |
| LlamaIndex BM25 | 48.5% | 9.2 |
| txtai（稠密+BM25） | 48.3% | — |
| Cognee（取回原文块） | 43.9% | 3.3 |

**重复测量的极差最高 9.2 个点，所以小于这个幅度的差距不能当作差异。** 检索是
确定性的，波动全部来自作答与判分的 LLM。据此只有两条结论站得住：

1. Cognee 的图谱路径显著领先。同一个系统、同一份图，取回原文块 43.9%（垫底）
   对图谱自答 64.4%（第一）——二十个点全部来自「用抽取出的结构作答」而非「返回
   原文」，与检索质量无关（它的检索是所有系统里最差的）。
2. 中间四家（我们、mem0、txtai、BM25）互相之间没有可断言的差距。

对照组说明模型本身值约 5 个点（同一份检索结果，换 k3 作答从 49.2% 升到
54.4%），所以 Cognee 的真实领先约十个点而非十五个。

### 抽取层（`extraction_layer.py`）

上面那张表把 OpenKimo 当成纯检索器测，而产品里存的从来不是原文轮次，是抽取并经
用户批准的事实。补上这一层的测量：

| 变体 | 抽出事实 | 准确率 |
|---|---|---|
| 线上提示词，上限 5 | 115 | 6.7% |
| 线上提示词，上限 12 | 141 | 7.5% |
| 改写提示词，上限 12 | 454 | 44.2% |

线上提示词抽的是永恒特质（「Caroline 是跨性别女性」），**一个日期都没有**——
它明写着排除事件（"Exclude anything tied to this conversation: what was done"）。
LoCoMo 问的全是「什么时候发生了什么」，所以 6.7% 说明的是瞄准的目标不同，不是
管线坏了。

有价值的是第三行：换成面向事件的提示词后是 44.2%，而**直接检索原文轮次是
49.2%**——在噪音范围内持平。也就是说「把对话压成扁平事实句」这件事本身不带来
增益。Cognee 领先靠的是我们没有的两样东西：实体与关系的**结构**（519 节点 /
1262 边，我们是 454 条互不相连的句子），以及**时间锚定**（条目内容里带事件发生
时间，而我们只有记录时间）。


## 完整对比（2026-08-19 更新）

同一份 LoCoMo（2 段对话、788 轮、120 题），同一个判分器，所有系统喂完全相同的
带时间戳原文。

| 方案 | 端到端准确率 | 说明 |
|---|---|---|
| Cognee（图谱自答） | **64.4%**（6 次均值） | 用抽取出的实体关系作答 |
| *OpenKimo — 换 k3 作答（对照）* | *53.3%* | 同一份检索结果，只换作答模型 |
| MemOS（取回内容） | 50.8% | LLM 抽取入库 + 纯稠密向量检索 |
| mem0（qdrant 混合） | 49.2% | 向量 + BM25 |
| txtai（稠密+BM25 混合） | 48.3% | |
| LlamaIndex BM25 | 48.3% | |
| OpenKimo（FTS5 + IDF scan） | 46.7% | |
| Cognee（取回原文块） | 45.0% | 同一个 Cognee，不走图谱 |

**重复测量的极差最高 9.2 个点**，所以只有两条结论站得住：

1. **Cognee 的图谱路径是唯一显著领先的**。扣掉作答模型带来的约 5 点，真实领先
   约十点。而它的检索是全场最差（45.0%）——那二十点全部来自「用抽取出的结构
   作答」而非「返回原文」。
2. **中间五家分不出高下，而它们用的技术互不相同**：

   | | 检索方式 |
   |---|---|
   | mem0 52.9% | 稠密向量 + BM25 混合 |
   | MemOS 50.8% | LLM 抽取入库 + 纯稠密向量 |
   | OpenKimo 48.8% | 纯词法（FTS5 三元组 + IDF 子串扫描） |
   | LlamaIndex 48.4% | 纯词法（BM25） |
   | txtai 48.3% | 稠密向量 + BM25 混合 |

   纯词法、纯向量、混合、以及带 LLM 抽取的，全部落在 48–53% 这个五点宽的带里，
   而同一系统重复测量的极差是 9.2 点。**换哪种检索技术都到不了这个带以外** ——
   天花板属于「在对话轮次上做检索」这个范式，不属于某种具体实现。

   我们自己的证据与之一致：今天把中文召回从 47.7% 提到 58.9%（+11 点），端到端
   一点没动。

   （更正：此表早先一版把 MemOS 写成「与我们同技术路线（FTS5 + 向量）」。那来自
   调研阶段一条未经验证的笔记，描述的是它的另一种配置；实际跑的 `general_text`
   后端源码是「嵌入查询、搜向量库」，无词法成分，而我们没有向量。结论不受影响，
   反而更强——共同点不是技术栈，是范式。）

### 没有跑成的，以及原因

| | 阻碍 |
|---|---|
| Graphiti | 嵌入式 Kuzu 后端建图大面积失败（10 轮只出 2 节点 0 边），需 Neo4j + Java |
| Zep | 瘦客户端，需服务端或云 API key |
| Letta | 机制是「对话中 agent 自改写 core memory」，批量灌入测不到 |
| Memori | 机制是拦截 LLM 调用捕获记忆，无公开的批量灌入接口 |
| SimpleMem | 装得上也跑得通，但逐轮串行调 LLM：**64 分钟只答完 20/120 题**，全量需数小时 |
| TencentDB | 未尝试。需起三个 Node 服务，本机 Node 22 具备 |

后四行不是能力评价，是**这套 benchmark 与它们的机制不匹配**，或者代价与信息量
不成比例。用批量灌入去测「对话中维护记忆」的系统，和早先用轮次召回去测图谱是同
一类错误。

### 三处必须连着看的限定

**配置选择两次改变了结论，幅度大于方案之间的差距。** mem0 用其文档推荐的 chroma
时会自行关闭混合检索（英文 68.5% → 58.3%）；中文首次用英文专用嵌入模型加默认
0.1 相似度阈值，它只有 11.6%，据此曾得出「我们中文领先 37 点」，修正后是落后。

**这套 benchmark 测不到本项目的差异化**：零依赖（2 个 JSONL + 1 个可删缓存，
对面 mem0 装出 353MB、txtai 环境 971MB）、写入需用户批准、行为类记忆无需查询即
注入。三条一条都没进表。

**端点怪癖消耗的时间超过任何一次调参。** k3 只接受 `temperature=1`（撞了三次：
对照组全零、cognee 的 tool_choice、SimpleMem 全部写入失败），DeepSeek 不支持
`json_schema`，LiteLLM 要求模型名带 provider 前缀。每一次的表现都是「跑完了，
结果是空的」。


## 这里的检索代码值不值它的体积（`bm25_with_cjk.py`）

LlamaIndex 的 BM25 在英文上与本项目打平，代码量约为三分之一，所以合理的疑问是：
六百多行买到了什么？中文上它 2.3%、我们 58.9%，但这不是答案——**没人需要写六百
行来修这件事**，一个把中文切成双字的分词器约五行。

先说一处失败的测法：`BM25Retriever.from_defaults(tokenizer=...)` 会打印
「deprecated」然后**静默忽略该参数**，结果与默认分词一位不差。若不是数字完全相
同引起怀疑，就会得出「换中文分词也救不回来」——一个恰好对我们有利的错误结论。
改为在喂入前把中文重写为空格分隔的双字，让 LlamaIndex 自带的词干化 BM25 正常
工作。

| 中文 | recall@1 | recall@3 | recall@8 |
|---|---|---|---|
| OpenKimo | **32.8%** | **46.4%** | **58.9%** |
| LlamaIndex BM25（原样） | 0.7% | 2.3% | 2.3% |
| LlamaIndex BM25 + 中文双字预切 | 22.2% | 37.7% | 51.3% |

| 英文 | recall@1 | recall@3 | recall@8 |
|---|---|---|---|
| OpenKimo | **36.4%** | **52.6%** | **64.6%** |
| LlamaIndex BM25 | 30.8% | 47.4% | 60.6% |

结算下来：

- **中文能力的主体不是这些代码买的。** 2.3% → 51.3%（49 个点）来自一个五行的
  分词决定；本项目额外的机制（IDF 加权、语言分流、倒排表）再加 7.6 点。
- **但那 7.6 点是真的。** 检索召回是确定性测量，不受端到端指标那 9.2 点的 LLM
  波动影响；两个独立的 BM25 实现（手写的与 LlamaIndex 的）在中文上都落在
  51.3%，互相印证。
- **它换不成答案质量。** 端到端上本项目 48.8%、LlamaIndex BM25 48.4%，分不出
  高下——与「中文召回 +11 点、端到端纹丝不动」是同一个现象。

所以诚实的定位是：检索确实比最好的 BM25 强，强得可测但不足以改变答案；这些代码
真正值钱的地方是零依赖（仅标准库 `sqlite3`）与增量索引（两万条时写入后再搜索
50 ms，全量重建则是 690 ms），而这两条没有任何一个 benchmark 在测。


## 这些问题里，有多少真的需要记忆（`question_shapes.py`）

Cognee 的图谱路径领先约十个点，来源是在密集叙事上把几轮内容合成为一个答案。要不
要投入建图，取决于一件 benchmark 回答不了的事：**这里实际被问的问题是什么形状**。
默认它会平移，是今天已经犯过两次的错误，所以改为去数。

对 243 条真实用户发言分类（本机 Claude Code 会话记录，等距取样）：

| 类别 | 占比 | 记忆在其中的角色 |
|---|---|---|
| 指令 / 任务请求 | **89.3%** | 行为类记忆**隐式生效**：长期约定、偏好、禁令 |
| 问单条事实 | 8.2% | 检索 |
| 问需组合两条以上的事实 | 2.5% | 图谱能帮上的那一类 |

第一版判据宽松，给出 14.0%；逐条看过之后发现它把「组合眼前的两件事」也算进去了
——问当前这次对话的进展、报 bug、让助手重读一段代码，这些上下文里就有，不走记忆
系统。收紧判据后是 2.5%，再人工核对那 6 条，真正需要组合两条**已存**记忆的约
1%。三种口径的结论一致。

两个后果：

1. **不建图谱。** 那十个点属于一种在这里几乎不出现的问题形态。
2. **优先级要改。** 89% 的发言是「去做某事」，而记忆在这些场景里从不被检索——它
   是被无条件注入的那部分行为类记忆。检索服务的是 8.2%。因此**行为类记忆约 121
   条的注入上限，重要性高于检索与图谱之和**：它是唯一服务于绝大多数交互的机制，
   而它现在只有退役和取代建议，自动归并仍未实现。


## 代价对比（`cost.py`）

准确率已经量到头了，各方案差距在噪音带内。代价此前一次都没量过——而「零依赖、
写入不触发重建」正是本项目一直宣称的优势，在测过之前它只是个说法。

788 轮原文、40 次查询，同一台机器。

| 方案 | 建库 | 查询中位 | 索引磁盘 | 写一条后再查 | 安装体积 |
|---|---|---|---|---|---|
| **OpenKimo (SQLite FTS5)** | **0.0 s** | 0.5 ms | 1.0 MB | **1.7 ms** | **0**（标准库 + 已有的 pydantic） |
| LlamaIndex BM25 | 0.1 s | **0.1 ms** | 0.7 MB | 需重建 | ~970 MB 环境 |
| txtai（稠密+BM25） | 1.5 s | 8.5 ms | 2.0 MB | 需重建 | ~970 MB 环境 |
| mem0（qdrant，关闭抽取） | 30.5 s | 39.5 ms | 3.7 MB | — | 360 MB 依赖树 |

需要 LLM 建库的三个不在同一量纲，它们的耗时是账单而非跑分：

| 方案 | 788 轮建库 + 提问 | 备注 |
|---|---|---|
| Cognee | 约 20 分钟 | 每 20 轮一次图谱抽取 |
| MemOS | 数分钟 | 每 20 轮一次抽取 |
| SimpleMem | **64 分钟只答完 20/120 题** | 逐轮串行调 LLM |

两点：

1. **「写入不触发重建」是真的，也只有我们有。** BM25 与 txtai 的索引在内存里，
   加一条记忆要整体重建；我们是磁盘上的 SQLite，写后再查 1.7 ms。真实使用中记忆
   是一条条攒出来的，这个差别比任何召回率都更常被触发。
2. **体积差三个数量级。** 记忆检索这一层，我们的第三方依赖是零——`sqlite3` 在标
   准库里，`pydantic` 本来就在。对面最小的 mem0 也要 360 MB 依赖树，其余环境接近
   或超过 1 GB。对一个要打包分发的桌面应用，这不是风格问题。

反过来说清楚：**BM25 查询比我们快五倍**（0.1 ms 对 0.5 ms），两者都远低于任何会
被察觉的阈值；这一栏我们没有优势，只是都够快。


## 汇总表

三个维度放在一起。`—` 表示没测，`N/A` 表示该指标对它不适用。

### 质量

| 方案 | 端到端准确率 | n | 召回@8 英文 | 召回@8 中文 |
|---|---|---|---|---|
| Cognee（图谱自答） | **64.5%** | 7 | N/A | N/A |
| *OpenKimo — 换 k3 作答（对照）* | *54.3%* | 6 | — | — |
| mem0（qdrant 混合） | 52.9% | 7 | **68.5%** | 49.3% |
| MemOS | 50.8% | 1 | **69.5%**\\* | — |
| **OpenKimo** | 48.8% | 7 | 64.6% | **58.9%** |
| LlamaIndex BM25 | 48.4% | 7 | 60.6% | 2.3% |
| txtai（稠密+BM25） | 48.3% | 2 | 57.6% | **61.9%** |
| Cognee（取回原文块） | 44.0% | 7 | 75.4%\\*† | — |
| mem0（纯向量，多语言） | — | — | 58.3% | 54.6% |
| LlamaIndex BM25 + 中文双字预切 | — | — | 60.6% | 51.3% |

端到端一栏的可分辨阈值是 **9.2 个点**（同一系统重复测量的最大极差），所以除
Cognee 图谱路径外，其余各行互相之间没有可断言的差距。召回一栏是确定性测量，不受
此波动影响。

Cognee 的两条路径都不返回原文轮次，无法用证据轮召回评分——这正是把主指标换成端到
端问答的原因。

### 代价

| 方案 | 建库 | 查询中位 | 索引磁盘 | 写一条后再查 | 安装体积 |
|---|---|---|---|---|---|
| **OpenKimo** | **0.0 s** | 0.5 ms | 1.0 MB | **1.7 ms** | **0** |
| LlamaIndex BM25 | 0.1 s | **0.1 ms** | 0.7 MB | 需重建 | ~970 MB 环境 |
| txtai | 1.5 s | 8.5 ms | 2.0 MB | 需重建 | ~970 MB 环境 |
| mem0（关闭抽取） | 30.5 s | 39.5 ms | 3.7 MB | — | 360 MB 依赖树 |
| Cognee | 约 20 分钟 | — | — | — | ~1.0 GB 环境 |
| MemOS | 数分钟 | — | — | — | ~1.2 GB 环境 |
| SimpleMem | 64 分钟未跑完 | — | — | — | ~1.4 GB 环境 |

后三行的建库时间发生在 LLM 里，是账单不是跑分，与前四行不同量纲。

### 一句话

质量上我们与 mem0、txtai、BM25、MemOS 无法区分，落后 Cognee 图谱路径约十点（扣除
作答模型差异后）；中文召回是全场第二（58.9%），英文第二（64.6%）；代价上建库、
增量写入、安装体积三项领先一到三个数量级，查询延迟比 BM25 慢五倍但均在无感区间。


## 补上的两格召回（`recall_from_saved_runs.py`）

两者的运行都写下了取回内容，所以这不需要重跑，只需把返回的文本映射回轮次 id。
但它们回答的是每段前 60 题，而召回表其余各行用的是全部 302 题——跨题集比排名比的
是题目难度，所以本项目的基线在同一批题上重算了一遍：

| 同一批 118 题 | recall@1 | recall@3 | recall@8 |
|---|---|---|---|
| OpenKimo | 34.7% | 51.7% | 61.9% |
| **MemOS**（返回 8 条原文轮次） | **39.0%** | **55.9%** | **69.5%** |
| Cognee 取回原文块 | 22.0% | 48.3% | 75.4% † |

**\*** 这两行来自 118 题子集，与表中其余各行（302 题）不可直接并列。

**†** 不是同一预算。Cognee 的一个「结果」是二十轮的文档块，八个结果就是一百六十
轮，别人是八轮。同预算下无法比较，这个数字只说明它的块里装得下答案。

MemOS 的召回高出本项目 7.6 点，是真实差距——同题集、确定性测量。而它的端到端是
50.8% 对 48.8%，**仍在 9.2 点的噪音带内**。今天第三次出现同一现象：召回涨了，答案
没变。


## SimpleMem 跑完了，它改写了这张表（2026-08-19 晚）

两小时后收工，120 题全部有答案。**75.0%，比 Cognee 高十点，是全场第一。**

汇总（重复测量的最大极差 10.0 点）：

| 方案 | n | 均值 | 区间 | 查询时是否做 LLM 推理 |
|---|---|---|---|---|
| **SimpleMem**（规划 + 反思，自答） | 1 | **75.0%** | — | **是，多轮** |
| **Cognee**（入库建图，自答） | 8 | **64.6%** | 63.3–65.8 | **是，图上生成** |
| *OpenKimo — 换 k3 作答（对照）* | 7 | *54.5%* | 52.5–55.8 | 否 |
| mem0（qdrant 混合） | 8 | 52.7% | 49.2–55.8 | 否 |
| OpenKimo | 8 | 49.4% | 46.7–53.3 | 否 |
| LlamaIndex BM25 | 8 | 48.0% | 43.3–52.5 | 否 |
| MemOS | 2 | 47.0% | 43.3–50.8 | 否 |
| txtai（稠密+BM25） | 3 | 46.6% | 43.3–48.3 | 否 |
| Cognee（取回原文块） | 8 | 43.0% | 35.8–45.8 | 否 |

**最后一列是这张表真正的分界线。** 之前把差距归因于「用结构作答」，那只解释了
Cognee；SimpleMem 存的不是图谱而是压缩后的条目，它领先靠的是查询时的
`[Planning]` 与 `[Intelligent Reflection]` 多轮循环——检索一次不够就再检索一次。

按这条线切开：

- **查询时做 LLM 推理的两家**：64.6% 与 75.0%
- **不做的七家**：43.0% 到 54.5%，彼此在噪音带内

存什么（原文 / 扁平事实 / 实体关系）、怎么检索（词法 / 向量 / 混合）都**不能**预测
准确率——这两条轴上的方案在下面那一组里混着排。能预测的只有查询时花不花 LLM。

代价那一面同样极端：SimpleMem 逐轮串行调用 LLM 入库，788 轮加 120 题跑了约两小时，
而本项目建库 0.0 秒。它买到的十点是用两个数量级的时间换的。

SimpleMem 自答用的是 DeepSeek，与本项目作答用的是同一个模型；Cognee 自答用 k3，
按对照组该扣约 5.5 点。所以 SimpleMem 对 Cognee 的领先是被低估而非高估的。


## 每轮自动检索能不能成立（`turn_retrieval.py`）

一个设计提案：不再在会话开场注入固定索引，而是每轮按相关性检索。动手之前没有
基线——`search` 在真实会话里几乎从不被调用（7 个会话 76 次工具调用中，Memory 被
调过 1 次，且是 `add`），所以「改好了没有」无从判断。

记忆库用本人真实会话按线上抽取提示词生成（19–25 条），发言取 90 条真实用户输入，
每轮取 top-3：

| | |
|---|---|
| ① 库里存在能实质帮上忙的记忆 | 58 / 90 = **64.4%** |
| ② 其中检索捞到了 | 19 / 58 = **32.8%** |
| ③ 库里没有、检索仍返回内容 | 22 / 32 |
| 自动注入的话 | 共 166 条，判定有用 29 条 = **17.5%** |

① 比预期高：即使 89% 的发言是指令，近三分之二的轮次库里仍有能帮上忙的东西。
② 是真瓶颈：19 条的库里都捞不到该捞的，不是规模问题是匹配方式问题。
③ 是自动注入的直接代价：每轮塞三条，平均 2.5 条是噪音，而且每轮都付。

### 试过并撤回：覆盖率下限

想用「命中权重占查询可覆盖权重的比例」做准入，低于门槛就返回空。实现后扫描：

| 门槛 | 中文召回@8 | 英文召回@8 | 真实发言仍有返回 |
|---|---|---|---|
| 0.00 | 55.3% | 61.3% | 78.9% |
| 0.20 | 55.3% | 61.3% | 78.9% |
| 0.30 | 53.0% | 61.3% | 75.6% |
| 0.50 | 49.7% | 61.3% | 73.3% |

**压不下噪音，只压召回**：门槛 0.50 时返回率降 5.6 点，中文召回也降 5.6 点，一比
一。0.20 这一档则一个格子都没改变。已撤回。

原因值得记住：那些「库里没有相关记忆却仍返回」的轮次，返回的条目**确实匹配了实
词**，只是匹配在语义上没有意义。**字面覆盖率看不见语义错配，任何字面阈值都修不
了它。**

这对提案的意义是直接的：每轮自动注入要成立，前提是能判断语义相关，而这需要嵌入
模型——正是被零依赖这条约束排除掉的东西。在补上语义能力之前，自动注入会把 17.5%
的有用率摊到每一轮。

实现中另有一处自伤：`floor: float = _COVERAGE_FLOOR` 把常量绑成默认参数，而默认
参数在函数定义时求值一次，于是扫描每个门槛得到的结果完全相同。改为调用时读取才
拿到上表。


## 要不要为语义检索付这个代价（`dense_vs_lexical.py`）

上一节的结论是「每轮自动注入要成立，前提是能判断语义相关」，而语义相关需要嵌入
模型。这一节把那个前提本身测了。

同一记忆库、同一批真实发言、同一个判分器，只换检索器：

| 检索器 | 有用率（第一次 / 第二次） | 建库 | 每次查询 | 依赖体积 |
|---|---|---|---|---|
| 词法（现状） | 14.0% / 15.3% | 5 ms | 0.4 ms | 0 |
| 稠密向量（多语言 MiniLM） | 18.1% / 13.7% | 627 ms | 9.1 ms | 183 MB |

**两次运行排序翻转。** 加上更早一次词法测得的 17.5%，两者区间完全重叠
（词法 14.0–17.5，稠密 13.7–18.1）。差异小于判分器自身的波动。

按位置拆开更清楚：

| | 第1条 | 第2条 | 第3条 |
|---|---|---|---|
| 词法 | 19.7% | 10.2% | 13.5% |
| 稠密向量 | 13.3% | 13.3% | 14.4% |

稠密向量三个位置几乎相同——它排出的顺序在这批数据上不携带有用信息。

两个否定结论：

1. **不加嵌入模型。** 183 MB、建库慢 150 倍、查询慢 25 倍，换一个测不出来的差异。
2. **不做每轮自动注入。** 无论哪个检索器，约 85% 的注入内容无用，而这是每轮都付。

那么「无论何时都按相关性检索」这个设想，结构上仍然成立，但它需要的判断能力现有
手段都提供不了：字面阈值修不了语义错配，嵌入模型也没打出差距。

### 唯一没被否掉的杠杆

让模型在该用 `search` 的时候真的去用。库里 64.4% 的轮次存在能帮上忙的记忆，而
`search` 在七个真实会话里被调用零次。这是提示词与可见性的问题——不花钱、不加依
赖、不改架构，而且「我现在需要查一下吗」正是模型比阈值和余弦相似度都更擅长判断
的事。


## 让检索被用起来（`search_usage.py`）

今天所有测量之后唯一没被否掉的杠杆，也是最便宜的一个：**库里 64.4% 的轮次存在能
帮上忙的记忆，而 `search` 在真实会话里被调用零次。**

原因在提示词里看得很清楚：整篇 `description.md` 是围绕「怎么写」组织的——kind 怎
么选、什么时候存、合并语义占了大半，「怎么读」只有中间一句话。模型把它当写入工具
用，是被这么教的。

改动是加一节「Looking something up」并放在「Kinds」之前，给三个能真正触发的条件：

- **在重新推导之前**——要读文件、grep、跑命令去确认某件事怎么工作／在哪里／当初
  怎么定的？先查。一次查询比它省下的工具调用便宜，而且答案是用户批准过的那个。
- **当用户把某事当成已定**（「上次说的那个路径」「按之前的约定」）——去查，而不是
  让他重说一遍。
- **在说「我不知道」之前**。

外加一句反向配重：**不要每轮都查**。多数发言是指令，没什么可查；而这个规模的库对
几乎任何查询都会返回东西，其中约六分之一有用。查询要由触发条件驱动，不是反射。

`search_usage.py` 从产品本来就会写的会话文件里统计调用率，不需要埋点。**当前基线
是 0 次**，改动有没有效，看这个数字有没有离开零。


## 阈值必须在真实库上验过

同一个错误在这里犯了三次，形状完全一样：在合成数据上调出一个数，在真实数据上它
不成立——而且三次都失败得很安静。

| 阈值 | 校准用的数据 | 真实数据 | 后果 |
|---|---|---|---|
| 高频 needle 剪枝 5% | 两千条的 benchmark | 记忆库只有几十条 | 5% 不足 1，**凡是真正匹配上的词全被剪掉**，搜索返回空 |
| 取代相似度 0.50 | 单句规则（6–10 个 token） | 850–1040 字符的多步骤流程 | 最明显的重复对只有 0.198，**检测器一对都抓不到** |
| 行为类上限 121 条 | 66 字符的合成条目 | 真实条目平均 576 字符 | 真实上限约 **14 条**，「三年后的墙」其实就在眼前 |

三次都不是数值调得不好，是**合成数据在某个没人检查过的属性上与真实数据不同**——
库的规模、条目的长度、条目的形状。而三次的表现都是「跑完了，没报错，什么也没发
生」。

还有一次形状略有不同但同源：取代检测的数字守卫是从 `dedup.may_merge` 抄来的，而
那道守卫服务的是**写入时的自动合并**——合并会静默丢掉旧文本，所以数字不同必须一票
否决。取代只是**给用户看的建议**，代价完全不对称：错误的否决永远藏起一个真重复，
错误的建议只花一行去拒绝。**把一个为破坏性操作设计的守卫套到非破坏性操作上**，同
样会安静地什么都不做。

所以定阈值之前，三步：

1. **说清楚这个阈值依赖数据的哪个属性**（规模？长度？语言？形状？）。说不清楚，
   就说明还不知道自己在调什么。
2. **去真实库上量那个属性**。`persistent.jsonl` 就在那里，一条命令的事。
3. **优先选没有断崖的度量**。「按频率加权」在任何库大小下都退化得平滑，「高于 5%
   就剪掉」在小库上直接翻转语义——同样的目的，一个有悬崖一个没有。

以及从守卫那次学到的第四条：**照搬一个阈值时，先确认它原来服务的操作和现在这个，
代价是不是同一个方向。**


## 那 10 点噪音，有多少是判分器（`rejudge.py`）

端到端表里同一系统重复测量的极差最高 10 点，此前一直被整体归给「判分器波动」。
对自答型系统这个可以直接分离：它们的答案存在文件里、固定不变，所以重复判分只暴露
判分器自己。

各判 5 轮：

| 方案 | 均值 | 标准差 | 极差 | 各轮 |
|---|---|---|---|---|
| SimpleMem（自答） | **76.3%** | 1.73 | 4.2 | 77.5 / 77.5 / 76.7 / 73.3 / 76.7 |
| Cognee（图谱自答） | **64.3%** | 0.37 | 0.8 | 64.2 ×4 / 65.0 |

两件事：

**归因错了。** 判分器只贡献 0.8–4.2 点，而检索型系统的重复测量能晃 10 点——差额来
自它们**每轮的作答本身也在变**（同样的检索结果，作答模型给出不同答案）。自答型系
统没有这一层，所以它们的重复测量稳定得多。

**SimpleMem 的领先是实的。** 76.3% 对 Cognee 64.3%、对本项目约 49%，差距远大于任
何一层噪音。原先 n=1 的顾虑解除，「查询时做 LLM 推理」这一轴可以当作可行动的结论。


## 记忆会不会过期（`incremental.py`）

其余每个 benchmark 都是「灌入固定语料，然后提问」，测的是检索，**测不到维护**——
而这个项目的设计赌注全在维护那一侧：取代、时间锚定、退役、批准。mem0 在 2026-04
放弃 UPDATE/DELETE 整合、改成单遍累积，赌的正是反面。两边都没有数据。

协议：12 个会话按时间顺序写入，9 条事实中 6 条中途被修订，最后问**当前值**。取材
自真实工作域（求解器、端口、文档路径、分支名、长期约定），因为条目形状已被证明会
决定成败。每个系统走它自己真实的写入路径——本项目是抽取 + `upsert_entry`（含去重
合并），mem0 是 `infer=True`（它的累积决策所在）。审批在此绕过：被测的是维护机
制，不是守着它的那道同意。

| 方案 | 当前值 | 旧值 | 两者并列 | 从未变更的留存 |
|---|---|---|---|---|
| **OpenKimo** | **6/6** | 0 | 0 | **3/3** |
| mem0（infer=True） | 4/6 | 1 | 1 | 1/3 |

mem0 在求解器上返回了已被取代的 SLSQP，在 TDI 上把新旧值一起给出而不指明哪个现
行；更要紧的是留存 1/3——两条从未变更的长期约定在它库里消失了。

### 赢的原因不是设计时以为的那个

**去重合并一次都没触发，而且按设计就不该触发**：数字不同是 `may_merge` 的一票否
决项，而 `5494` 对 `8721`、`SLSQP` 对 `AdaGrad` 正是这种情况。

6/6 靠的是**每条都带日期**：

    截至 2026-03-04，TDI 估计使用 48 小时滚动窗口，deque 存 576 个采样点。
    2026-05-06 起，TDI 改用 EWMA，alpha=1/288，不再维护 48 小时窗口。

两条都在库里，谁也没删，但读的人能排出先后。时间锚定是为「什么时候决定的」加的，
顺带解决了时效性——这一层此前没有预料到。

于是归并的定位要改：

| | 由谁解决 |
|---|---|
| 时效性（返回的是不是当前值） | **时间锚定**，已解决 |
| 容量（库随每次变更增长） | **归并，未解决** |

12 个会话产生 15 条，而行为类上限约 14 条。**变更越频繁，撞墙越快**——归并要防的
是这个，不是过期。


### 据此改了取代检测

增量语料暴露了一个更强的信号：**修订会自己说出来**。「2026-05-06 起，TDI 改用
EWMA，不再维护 48 小时窗口」不是与旧条目相似，而是明说取代了它。相似度只能猜，而
且猜错了——六处真实修订，相似度一处都没配上，因为修订通常简短、与被它取代的原文
共享的措辞很少。

改动后：增量语料 0 → **5/6**，真实库仍是 2 对（无回归）。

漏掉的第六条是文档路径变更：新旧路径字符串完全不同，主题重叠为 0，词法上确实分辨
不出两条讲的是同一件事。这是诚实的极限，没有为凑满而调参。

过程中撞到一个自相矛盾，值得记下来：**用来识别「宣告取代」的词，正是否定极性守卫
要否决的东西**。宣告替换的惯常写法就是否定被替换的那个（「不再维护」「no longer
uses」），于是守卫把它读成意义反转，三处真实修订里有两处栽在这里。

修法是原则性的而非调参：判断否定极性时先去掉宣告用语。「always run the migration」
对「never run the migration」不含任何宣告，去掉之后否定依然可见，仍被正确否决。
