...
## 这些问题里，有多少真的需要记忆（`question_shapes.py`）

Cognee 的图谱路径领先约十个点，来源是在密集叙事上把几轮内容合成为一个答案。要不
要投入建图，取决于一件 benchmark 回答不了的事：**这里实际被问的问题是什么形状**。
默认它会平移，是今天已经犯过两次的错误，所以改为去数。

对 243 条真实用户发言分类（本机 Claude Code 会话记录，等距取样）：

| 类别 | 占比 | 记忆在其中的角色 |
|---|---|---|
| 指令 / 任务请求 | **89.3%** | 行为类记忆**隐式生效**：长期约定、偏好、禁令 |
| 问单条事实 | 8.2% | 检索 |
| 问需组合两条以上的事实 | 2.5% | 图谱能帮上的那一类 |

第一版判据宽松，给出 14.0%；逐条看过之后发现它把「组合眼前的两件事」也算进去了
——问当前这次对话的进展、报 bug、让助手重读一段代码，这些上下文里就有，不走记忆
系统。收紧判据后是 2.5%，再人工核对那 6 条，真正需要组合两条**已存**记忆的约
1%。三种口径的结论一致。

两个后果：

1. **不建图谱。** 那十个点属于一种在这里几乎不出现的问题形态。
2. **优先级要改。** 89% 的发言是「去做某事」，而记忆在这些场景里从不被检索——它
   是被无条件注入的那部分行为类记忆。检索服务的是 8.2%。因此**行为类记忆约 121
   条的注入上限，重要性高于检索与图谱之和**：它是唯一服务于绝大多数交互的机制，
   而它现在只有退役和取代建议，自动归并仍未实现。


## 代价对比（`cost.py`）

准确率已经量到头了，各方案差距在噪音带内。代价此前一次都没量过——而「零依赖、
写入不触发重建」正是本项目一直宣称的优势，在测过之前它只是个说法。

788 轮原文、40 次查询，同一台机器。

| 方案 | 建库 | 查询中位 | 索引磁盘 | 写一条后再查 | 安装体积 |
|---|---|---|---|---|---|
| **OpenKimo (SQLite FTS5)** | **0.0 s** | 0.5 ms | 1.0 MB | **1.7 ms** | **0**（标准库 + 已有的 pydantic） |
| LlamaIndex BM25 | 0.1 s | **0.1 ms** | 0.7 MB | 需重建 | ~970 MB 环境 |
| txtai（稠密+BM25） | 1.5 s | 8.5 ms | 2.0 MB | 需重建 | ~970 MB 环境 |
| mem0（qdrant，关闭抽取） | 30.5 s | 39.5 ms | 3.7 MB | — | 360 MB 依赖树 |

需要 LLM 建库的三个不在同一量纲，它们的耗时是账单而非跑分：

| 方案 | 788 轮建库 + 提问 | 备注 |
|---|---|---|
| Cognee | 约 20 分钟 | 每 20 轮一次图谱抽取 |
| MemOS | 数分钟 | 每 20 轮一次抽取 |
| SimpleMem | **64 分钟只答完 20/120 题** | 逐轮串行调 LLM |

两点：

1. **「写入不触发重建」是真的，也只有我们有。** BM25 与 txtai 的索引在内存里，
   加一条记忆要整体重建；我们是磁盘上的 SQLite，写后再查 1.7 ms。真实使用中记忆
   是一条条攒出来的，这个差别比任何召回率都更常被触发。
2. **体积差三个数量级。** 记忆检索这一层，我们的第三方依赖是零——`sqlite3` 在标
   准库里，`pydantic` 本来就在。对面最小的 mem0 也要 360 MB 依赖树，其余环境接近
   或超过 1 GB。对一个要打包分发的桌面应用，这不是风格问题。

反过来说清楚：**BM25 查询比我们快五倍**（0.1 ms 对 0.5 ms），两者都远低于任何会
被察觉的阈值；这一栏我们没有优势，只是都够快。


## 汇总表

三个维度放在一起。`—` 表示没测，`N/A` 表示该指标对它不适用。

### 质量

| 方案 | 端到端准确率 | n | 召回@8 英文 | 召回@8 中文 |
|---|---|---|---|---|
| Cognee（图谱自答） | **64.5%** | 7 | N/A | N/A |
| *OpenKimo — 换 k3 作答（对照）* | *54.3%* | 6 | — | — |
| mem0（qdrant 混合） | 52.9% | 7 | **68.5%** | 49.3% |
| MemOS | 50.8% | 1 | **69.5%**\\* | — |
| **OpenKimo** | 48.8% | 7 | 64.6% | **58.9%** |
| LlamaIndex BM25 | 48.4% | 7 | 60.6% | 2.3% |
| txtai（稠密+BM25） | 48.3% | 2 | 57.6% | **61.9%** |
| Cognee（取回原文块） | 44.0% | 7 | 75.4%\\*† | — |
| mem0（纯向量，多语言） | — | — | 58.3% | 54.6% |
| LlamaIndex BM25 + 中文双字预切 | — | — | 60.6% | 51.3% |

端到端一栏的可分辨阈值是 **9.2 个点**（同一系统重复测量的最大极差），所以除
Cognee 图谱路径外，其余各行互相之间没有可断言的差距。召回一栏是确定性测量，不受
此波动影响。

Cognee 的两条路径都不返回原文轮次，无法用证据轮召回评分——这正是把主指标换成端到
端问答的原因。
...
| 方案 | 建库 | 查询中位 | 索引磁盘 | 写一条后再查 | 安装体积 |
|---|---|---|---|---|---|
| **OpenKimo** | **0.0 s** | 0.5 ms | 1.0 MB | **1.7 ms** | **0** |
| LlamaIndex BM25 | 0.1 s | **0.1 ms** | 0.7 MB | 需重建 | ~970 MB 环境 |
| txtai | 1.5 s | 8.5 ms | 2.0 MB | 需重建 | ~970 MB 环境 |
| mem0（关闭抽取） | 30.5 s | 39.5 ms | 3.7 MB | — | 360 MB 依赖树 |
| Cognee | 约 20 分钟 | — | — | — | ~1.0 GB 环境 |
| MemOS | 数分钟 | — | — | — | ~1.2 GB 环境 |
| SimpleMem | 64 分钟未跑完 | — | — | — | ~1.4 GB 环境 |

后三行的建库时间发生在 LLM 里，是账单不是跑分，与前四行不同量纲。

### 一句话

质量上我们与 mem0、txtai、BM25、MemOS 无法区分，落后 Cognee 图谱路径约十点（扣除
作答模型差异后）；中文召回是全场第二（58.9%），英文第二（64.6%）；代价上建库、
增量写入、安装体积三项领先一到三个数量级，查询延迟比 BM25 慢五倍但均在无感区间。


## 补上的两格召回（`recall_from_saved_runs.py`）

两者的运行都写下了取回内容，所以这不需要重跑，只需把返回的文本映射回轮次 id。
但它们回答的是每段前 60 题，而召回表其余各行用的是全部 302 题——跨题集比排名比的
是题目难度，所以本项目的基线在同一批题上重算了一遍：

| 同一批 118 题 | recall@1 | recall@3 | recall@8 |
|---|---|---|---|
| OpenKimo | 34.7% | 51.7% | 61.9% |
| **MemOS**（返回 8 条原文轮次） | **39.0%** | **55.9%** | **69.5%** |
| Cognee 取回原文块 | 22.0% | 48.3% | 75.4% † |

**\*** 这两行来自 118 题子集，与表中其余各行（302 题）不可直接并列。

**†** 不是同一预算。Cognee 的一个「结果」是二十轮的文档块，八个结果就是一百六十
轮，别人是八轮。同预算下无法比较，这个数字只说明它的块里装得下答案。

MemOS 的召回高出本项目 7.6 点，是真实差距——同题集、确定性测量。而它的端到端是
50.8% 对 48.8%，**仍在 9.2 点的噪音带内**。今天第三次出现同一现象：召回涨了，答案
没变。


## SimpleMem 跑完了，它改写了这张表（2026-08-19 晚）

两小时后收工，120 题全部有答案。**75.0%，比 Cognee 高十点，是全场第一。**

汇总（重复测量的最大极差 10.0 点）：

| 方案 | n | 均值 | 区间 | 查询时是否做 LLM 推理 |
|---|---|---|---|---|
| **SimpleMem**（规划 + 反思，自答） | 1 | **75.0%** | — | **是，多轮** |
| **Cognee**（入库建图，自答） | 8 | **64.6%** | 63.3–65.8 | **是，图上生成** |
| *OpenKimo — 换 k3 作答（对照）* | 7 | *54.5%* | 52.5–55.8 | 否 |
| mem0（qdrant 混合） | 8 | 52.7% | 49.2–55.8 | 否 |
| OpenKimo | 8 | 49.4% | 46.7–53.3 | 否 |
| LlamaIndex BM25 | 8 | 48.0% | 43.3–52.5 | 否 |
| MemOS | 2 | 47.0% | 43.3–50.8 | 否 |
| txtai（稠密+BM25） | 3 | 46.6% | 43.3–48.3 | 否 |
| Cognee（取回原文块） | 8 | 43.0% | 35.8–45.8 | 否 |

**最后一列是这张表真正的分界线。** 之前把差距归因于「用结构作答」，那只解释了
Cognee；SimpleMem 存的不是图谱而是压缩后的条目，它领先靠的是查询时的
`[Planning]` 与 `[Intelligent Reflection]` 多轮循环——检索一次不够就再检索一次。

按这条线切开：

- **查询时做 LLM 推理的两家**：64.6% 与 75.0%
- **不做的七家**：43.0% 到 54.5%，彼此在噪音带内

存什么（原文 / 扁平事实 / 实体关系）、怎么检索（词法 / 向量 / 混合）都**不能**预测
准确率——这两条轴上的方案在下面那一组里混着排。能预测的只有查询时花不花 LLM。

代价那一面同样极端：SimpleMem 逐轮串行调用 LLM 入库，788 轮加 120 题跑了约两小时，
而本项目建库 0.0 秒。它买到的十点是用两个数量级的时间换的。

SimpleMem 自答用的是 DeepSeek，与本项目作答用的是同一个模型；Cognee 自答用 k3，
按对照组该扣约 5.5 点。所以 SimpleMem 对 Cognee 的领先是被低估而非高估的。


## 每轮自动检索能不能成立（`turn_retrieval.py`）

一个设计提案：不再在会话开场注入固定索引，而是每轮按相关性检索。动手之前没有
基线——`search` 在真实会话里几乎从不被调用（7 个会话 76 次工具调用中，Memory 被
调过 1 次，且是 `add`），所以「改好了没有」无从判断。

记忆库用本人真实会话按线上抽取提示词生成（19–25 条），发言取 90 条真实用户输入，
每轮取 top-3：

| | |
|---|---|
| ① 库里存在能实质帮上忙的记忆 | 58 / 90 = **64.4%** |
| ② 其中检索捞到了 | 19 / 58 = **32.8%** |
| ③ 库里没有、检索仍返回内容 | 22 / 32 |
| 自动注入的话 | 共 166 条，判定有用 29 条 = **17.5%** |

① 比预期高：即使 89% 的发言是指令，近三分之二的轮次库里仍有能帮上忙的东西。
② 是真瓶颈：19 条的库里都捞不到该捞的，不是规模问题是匹配方式问题。
③ 是自动注入的直接代价：每轮塞三条，平均 2.5 条是噪音，而且每轮都付。

### 试过并撤回：覆盖率下限

想用「命中权重占查询可覆盖权重的比例」做准入，低于门槛就返回空。实现后扫描：

| 门槛 | 中文召回@8 | 英文召回@8 | 真实发言仍有返回 |
|---|---|---|---|
| 0.00 | 55.3% | 61.3% | 78.9% |
| 0.20 | 55.3% | 61.3% | 78.9% |
| 0.30 | 53.0% | 61.3% | 75.6% |
...
**压不下噪音，只压召回**：门槛 0.50 时返回率降 5.6 点，中文召回也降 5.6 点，一比
一。0.20 这一档则一个格子都没改变。已撤回。

原因值得记住：那些「库里没有相关记忆却仍返回」的轮次，返回的条目**确实匹配了实
词**，只是匹配在语义上没有意义。**字面覆盖率看不见语义错配，任何字面阈值都修不
了它。**

这对提案的意义是直接的：每轮自动注入要成立，前提是能判断语义相关，而这需要嵌入
模型——正是被零依赖这条约束排除掉的东西。在补上语义能力之前，自动注入会把 17.5%
的有用率摊到每一轮。

实现中另有一处自伤：`floor: float = _COVERAGE_FLOOR` 把常量绑成默认参数，而默认
参数在函数定义时求值一次，于是扫描每个门槛得到的结果完全相同。改为调用时读取才
拿到上表。
