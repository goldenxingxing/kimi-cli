...
- Web: Restore a way to set the global default model — the per-session model picker replaced the global one instead of joining it, and nothing else could set the default: the admin Models page shows which model is default but has no action to change it. Picking a model still binds it to the current session; setting the global default is a second action at the foot of the picker, and it again restarts the idle sessions, names the busy ones it skipped, and offers to restart those. Affects 0.1.39 only
- Web: Give a single session its own model — model choice was global, so trying a cheaper or a stronger model on one thread meant switching the whole workspace over and remembering to switch it back. A session now carries its own model, stored alongside the per-session overrides that were already there (thinking, agent spec); a session without one keeps following the global default, including when that default later changes. The picker sits with the plan and YOLO controls, and refuses to switch while the session is generating — the model is read when a turn starts, so a mid-turn switch would silently not apply
- Web: Stop one unresponsive browser tab from freezing a whole session — every message a session produced was written to each attached socket inline, on the task that drains the worker's stdout, so a client that stopped reading its socket stopped that read loop, filled the worker's stdout pipe, and blocked the worker mid-turn. The agent itself kept going and kept writing `wire.jsonl`, which is why reloading the page revealed a turn that had actually finished minutes earlier — the reload worked because it closed the wedged socket. Each connection now has its own send queue and writer task, so backpressure stays on the one connection it belongs to, and a client that falls hopelessly behind is dropped and left to reconnect and replay
- Web: Stay logged in while you are using it — a browser session was a fixed 24-hour window stamped at login, so a tab left open across the boundary was thrown back to the login page mid-use, and the first sign of it was a manual reload. Any request inside the window now slides the deadline forward once less than half of it is left, and an expired session takes the browser to the login page on its own instead of leaving a workspace that quietly does nothing. Two new settings bound it: `KIMI_WEB_SESSION_MAX_AGE` (idle window, default 24h) and `KIMI_WEB_SESSION_ABSOLUTE_MAX_AGE` (measured from login and never extended, default 30 days)
